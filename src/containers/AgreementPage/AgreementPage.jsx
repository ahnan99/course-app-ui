...
class AgreementPage extends Component {
  ...
  componentDidMount() {
    // this.update();
    this.props.actions.getAgreement({ username: this.props.application.username } )
  }
  ...
  render() {
    ...
    if (!this.props.user.agreementList.length) {
      return <h2>当前还没有培训协议书</h2>;
    }
    return (
      <Spin spinning={this.state.loading}>
        <div className="document" style={{ width: "100%" }} ref={this.divRef}>
          <div style={{ backgroundColor: "#F0FFFF" }}>
            <h2 style={{ textAlign: "left" }}>我的培训协议</h2>
          </div>
          {
            this.props.user.agreementList.map((agreement, index) => (
              ...
              <Col flex="none">
                <div>
                  <a href={(agreement.filename.indexOf("https://")===-1 ? axios.defaults.baseURL : "") + agreement.filename + '?response-content-type=application/octet-stream'} download target="_blank" style={{fontSize:'1.4em'}}>下载协议</a>
                </div>
              </Col>
            </Row>
            ))
